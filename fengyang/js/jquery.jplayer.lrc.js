...
(function($){
	$.lrc = {
		...
		lrcList: 'jp-lrc-list', /* 容器Class */
		hoverClass: 'jp-lrc-current', /* 选中节点的className */
		...
		if(typeof(txt) != 'string' || txt.length < 1) return;
			...
			if(this.list.length > 0) {
				...
				/* 赋值到指定容器 */
				$('.'+this.lrcList).html(html).animate({ marginTop: 0 }, 100).show();
				/* 定时调用回调函数，监听歌曲进度 */
		...
		start: function(callback) {
			...
			/* 有歌词则跳转到歌词时间轴 */
			if(this.hasLrc == 1) {
				//setInterval($.proxy(self.switchScreen, self), self.options.switchScreenDelay);
				this.handle = setInterval($.proxy(self.jump, self), this.interval*1000);
			}
		},
		/* 跳到指定时间的歌词 */
		jump: function(duration) {
			duration = this.callback();
			...
			this.pivot = pivot;
			tmpobj = $('.'+this.lrcList).children().removeClass(this.hoverClass).eq(pivot).addClass(thisobj.hoverClass);
