(function ($) {
    var defaults = {
        picsUL: 'idSlider3',
        iconsUL: 'idSliderIcons3',
		buttonsUL: 'idSliderButtons3',
        step: 3000,
        picWidth: 550,
		picHeight: 320
    };
	
    $.fn.picShow3 = function (options) {
    ...
    var XK = {
		initialize:function(options){
			...
			this._buttonsUL=$('#'+options.buttonsUL); //按钮容器
			...
			//控制按钮增加鼠标行为
			this._buttonsUL.children().first().click(function(){
				if(thisObj._index===0){
					var index=num-1;
					thisObj.picChange(index);
				}else{
					var index=thisObj._index-1;
					thisObj.picChange(index);
				}
			});
			this._buttonsUL.children().last().click(function(){
				if(thisObj._index===num-1){
					var index=0;
					thisObj.picChange(index);
				}else{
					var index=thisObj._index+1;
			...
			this._buttonsUL.hover(function(){
				thisObj.timeStop();
			},function(){
				thisObj.timeStart();
			});
