(function ($) {
    ...
    $.fn.picShow7 = function (options) {
        var options = $.extend(defaults, options);
		//此处this为jQuery对象
        return this.each(function () {
			//each中的this为DOM元素
			var XK=new ClassPicShow7($(this),options);
            XK.init();
        });
    };
	
	
	function ClassPicShow7(idObj,options){
		this._index=0; //当前索引
		...
		this._buttonsUL=idObj.find('.sliderButtons'); //控制按钮
		this._step=options.step;  //轮播时间间隔，单位毫秒
		this._timer=null;  //定时器
	}
	
	ClassPicShow7.prototype={
		init:function(){
			...
			//控制按钮增加鼠标行为
			this._picsUL.parent().hover(function(){
				thisObj.timeStop();
				thisObj._buttonsUL.show();
			},function(){
				thisObj.timeStart();
				thisObj._buttonsUL.hide();
			});
			...
			this._buttonsUL.find('.prev').on('click',function(){
				var index=thisObj._index;
				if(index===0){
					index=num-1
				}else{
					index--;
			...
			this._buttonsUL.find('.next').on('click',function(){
				var index=thisObj._index;
				if(index===num-1){
					index=0;
				}else{
